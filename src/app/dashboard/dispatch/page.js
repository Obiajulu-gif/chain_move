...
const DispatchPage = () => {
	...
	const assignDriver = (tripId, driverName) => {
		setTrips((prevTrips) =>
			prevTrips.map((trip) =>
				trip.id === tripId
					? { ...trip, driver: driverName, status: "In Progress" }
					: trip
			)
		);
	};
		...
		<div className="min-h-screen bg-gray-900 text-white p-6 md:p-8">
			...
			>
				...
				<p className="text-sm md:text-base text-gray-300 mt-2">
					Manage and assign drivers to active trips.
				</p>
			</motion.div>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{trips.map((trip) => (
					<motion.div
						key={trip.id}
						className={`p-4 md:p-6 rounded-lg shadow-lg transition-shadow duration-300 ${
							trip.status === "Pending"
						...
						<div className="mt-4">
							{trip.status === "Pending" && (
								<select
									...
									className="w-full bg-gray-700 text-white p-2 rounded-md text-sm md:text-base"
								>
			...
			{/* Driver List Section */}
			<motion.div
				className="mt-10 p-4 md:p-6 bg-gray-800 rounded-lg shadow-lg"
				initial={{ opacity: 0, y: 20 }}
				animate={{ opacity: 1, y: 0 }}
				transition={{ duration: 0.8 }}
			>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
					{drivers.map((driver) => (
						...
						>
							<h3 className="text-lg md:text-xl font-bold">{driver.name}</h3>
							<p className="text-sm md:text-base">
								Status:{" "}
								<span
									className={`font-semibold ${
										driver.available ? "text-green-300" : "text-red-300"
									}`}
								>
