...
const SidebarLink = ({ href, icon, label, badge, isSidebarOpen }) => (
  <motion.div
    ...
    <Link
      href={href}
      className="flex items-center justify-between p-2 rounded hover:bg-neutral-800 transition-colors">
...
const SidebarSection = ({ title, links, isSidebarOpen }) => (
  <div>
    {isSidebarOpen && title && (
      <motion.div
        className="text-sm text-neutral-400 mt-6 pt-4 border-t border-neutral-700"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
...
const Sidebar = () => {
  ...
  return (
    <motion.div
      className="bg-neutral-900 text-white max-h-screen p-4 flex flex-col justify-between fixed md:relative z-50 shadow-lg overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-700 scrollbar-track-neutral-900 scrollbar-thumb-rounded"
      style={{ height: "100vh" }}
        ...
        <nav className="mt-4 space-y-4">
          {profileLinks.map(({ href, icon, label }) => (
            <motion.div
              ...
              <Link
                href={href}
                className="flex items-center space-x-4 p-2 rounded hover:bg-neutral-800 transition-colors">
                <span className="text-2xl">{icon}</span>
                {isSidebarOpen && (
                  <span className="text-lg font-medium">{label}</span>
                )}
